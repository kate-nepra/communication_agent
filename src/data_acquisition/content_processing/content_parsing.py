...
class ExtendedSchema(BaseSchema):
    """ Extended schema for places and administrative entities """
    address: str
...
def get_parsed_content_by_function_call(agent: ApiAgent, url: str, content: str) -> BaseSchema:
    ...
    messages = [Message(role="system",
      ...
      2. Use the function with the most fitting description, pass parameters as described:
        ...
        If you encounter address of a place (such as address of a municipal office for administration or address of concert-hall for an event), assign is as address. Fill in "Not found" if address not found but required.
        For an event, assign date(s) of the event as list of durations. The duration format is a JSON object with fields "start" and "end". Field "end" is optional, it is used for period of time (that are two dates from-to, like startdate-enddate, for example 31 jan–14 feb 2024). Use the format YYYY-MM-DD for date, and format HH:MM:SS for time. For example {dates}.
