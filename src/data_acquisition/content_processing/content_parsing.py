...
from src.data_acquisition.constants import STATIC, PLACE, EVENT, ADMINISTRATION, SYSTEM, USER
...
def get_parsed_content_by_function_call(agent: ApiAgent, url: str, content: str) -> BaseSchema:
    ...
    messages = [Message(role=SYSTEM,
                        ...
                        For an event entity: assign date(s) of the event as list of durations. The duration format is a stringified JSON object {dates_format} with fields "start" and "end". Field "end" is optional, it is used for period of time (that are two dates from-to, like startdate-enddate, for example 31 jan–14 feb 2024). Use the format YYYY-MM-DD for date, and format HH:MM:SS for time. For example {dates_example}. 
                        3. End with the function call response, do NOT add any additional text. The function call must be in valid JSON format."""),
                Message(role=USER,
...
def get_parsed_by_type(type_name, agent: ApiAgent, url: str, content: str) -> dict:
    return agent.get_json_format_response(BaseSchema, [Message(role="system", content="")])  # todo
